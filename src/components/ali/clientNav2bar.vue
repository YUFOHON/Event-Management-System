<template>
  <nav class="navbar navbar-expand-lg bg-body-tertiary justify-content-between" id="cSecondbar">
    <div class="container-fluid">
      ...
      <div class="navPage">
        <a v-for="(a, index) in props.arr" :key="a.name" :href=a.URL>{{ a.name }} <a
            v-if="index < props.arr.length - 1">></a>
        </a>
      </div>
  ...
  setup(props, context) {
    ...
    const sortEvent = async () => {
      ...
      if (isSearchEvent.value) {
        ...
        router.push({ name: 'clientEventView', query: { page: 1, sort: sorting, category: category, input: route.query.input, startDate: startDate.value,endDate:endDate.value } });

      } else
        router.push({ name: 'clientEventView', query: { page: 1, sort: sorting, category: category, startDate: startDate.value,endDate:endDate.value } });
      //reset the category
      category = '';
    }
