<template>
  ...
  <RouterLink :to=" { name: 'eventRegister', params:{eventId:event._id,eventName:event.eventName} } " class="btn btn-primary float-right">註冊</RouterLink>
          ...
          <div class="text-middle" style="padding-left: 590px;"><button @click="routerTo"
              class="btn btn-primary float-right">Register</button></div>
...
export default {
  ...
  setup() {
    ...
    async function check_reg_record() {
      ...
      var response = await fetch("/api/events/check_reg_record/userID=" + userId);
      if (response.ok) {
    ...
    return {
      event, routerTo,check_reg_record
    };
